...
## Ownership statement
I am comfortable submitting this repository because I can explain the application structure, the two mid-course features, the tests, and the final release-readiness files. AI was used to draft and review parts of the work, but its output was not accepted automatically: findings were checked against files and commands, and at least one generated behavior was corrected after review. I re-ran the full test suite and verified the /health endpoint using the local application during the final packaging check. GitHub Actions was successfully verified on the final-project branch. Docker runtime verification could not be completed on this machine because Docker Desktop could not start without virtualization support, so I documented that limitation rather than claiming a result that was not observed.
